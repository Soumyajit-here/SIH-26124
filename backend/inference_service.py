...
MODEL1_PATH = r"C:\Users\soumy\OneDrive\Desktop\SIH-21624\best.pt"
...
class InferenceService:
    ...
    def initialize(self):
        ...
        if not os.path.exists(MODEL1_PATH):
            print(f"Warning: Model 1 not found at {MODEL1_PATH}. Inference will fail.")
            self.model1 = None
        else:
            self.model1 = YOLO(MODEL1_PATH)
            
        if not os.path.exists(MODEL2_PATH):
            print(f"Warning: Model 2 not found at {MODEL2_PATH}. Inference will fail.")
            self.model2 = None
        else:
